within NOODEL.Model;
model NOODEL_standalone "NOODEL as standalone model"

   inner Modelica.SIunits.Power Qb=(head.core.QB
       + chest.core.QB + back.core.QB + pelvis.core.QB + l_Shoulder.core.QB +
      r_Shoulder.core.QB + l_Arm.core.QB + r_Arm.core.QB + l_Hand.core.QB +
      r_Hand.core.QB + l_Thigh.core.QB + r_Thigh.core.QB + l_Leg.core.QB +
      r_Leg.core.QB + l_Foot.core.QB + r_Foot.core.QB + head.skin.QB + chest.skin.QB
       + back.skin.QB + pelvis.skin.QB + l_Shoulder.skin.QB + r_Shoulder.skin.QB
       + l_Arm.skin.QB + r_Arm.skin.QB + l_Hand.skin.QB + r_Hand.skin.QB +
      l_Thigh.skin.QB + r_Thigh.skin.QB + l_Leg.skin.QB + r_Leg.skin.QB +
      l_Foot.skin.QB + r_Foot.skin.QB) "Basal metabolic rate of whole body";
  inner Modelica.SIunits.Area   Stotal = head.skin.S+back.skin.S+chest.skin.S
          +pelvis.skin.S+l_Shoulder.skin.S+r_Shoulder.skin.S+l_Arm.skin.S+r_Arm.skin.S
          +l_Thigh.skin.S+r_Thigh.skin.S+l_Leg.skin.S+r_Leg.skin.S+l_Foot.skin.S
          +r_Foot.skin.S+l_Hand.skin.S+r_Hand.skin.S
    "surface area of whole body";

  // Evaporative heat transfer
  Modelica.SIunits.HeatFlowRate ESW_total
    "Total heat loss by evaporation of sweat";
  Modelica.SIunits.HeatFlux E_total_m2
    "Total evaporative heat loss per sqr meter";
  Modelica.SIunits.HeatFlowRate Qe;

  // Signals active system
  inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC WRMS
    "Integrated warm signal"                                                                   annotation(HideResult=true);
  inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC CLDS
    "Integrated cold signal"                                                                   annotation(HideResult=true);
  inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC Cerr(start=0) annotation(HideResult=true);
  inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC Cwarm(start=0) annotation(HideResult=true);
  inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC Ccold(start=-1) annotation(HideResult=true);
  inner Real ST "Vasoconstriction signal";
  inner NOODEL.NonSIUnits.FlowRate DL "Vasodilation signal";

  // Temperatures
  inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TskinAve
    "Mean skin temperature";
  Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TskinAve_seven_point
    "Seven point mean skin temperature according Hardy and DuBois";
  Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TsetAve
    "Mean set point temperature";
  inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TsetpsyAve
    "Mean psychological set point temperature";
    // Add TSkinAll and TCoreAll and TOuterLayerAll
   inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TSkinAll[16] "All local skin temperatures";
   inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TCoreAll[16] "All local core temperatures";
   inner Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TOuterLayerAll[16] "All local outer layer temperatures";

  // Heat flows
  inner Modelica.SIunits.HeatFlowRate Qshiv
    "Heat production by shivering";
  inner  Modelica.SIunits.HeatFlowRate Qmet;
  inner NOODEL.NonSIUnits.MetabolicRate met "Metabolic rate of whole body";

  //Psychological model
  Real TSmean( start=0) "Mean thermal sensation";
  Real TSlo[16] "Local thermal sensation" annotation(HideResult=true);
  inner Real TSo "Overall thermal sensation";
  Real TCmean "Local thermal comfort";
  Real TClo[16] "Local thermal comfort" annotation(HideResult=true);
  Real TCo "Local thermal comfort";

  inner Modelica.SIunits.Pressure PAIR;

  Interfaces.Adapters.TC TCl_head
    annotation (Placement(transformation(extent={{154,92},{166,104}})));
  Interfaces.Adapters.TS TSl_head
    annotation (Placement(transformation(extent={{74,92},{86,104}})));
  Interfaces.Adapters.TC TCl_chest
    annotation (Placement(transformation(extent={{154,72},{166,84}})));
  Interfaces.Adapters.TS TSl_chest
    annotation (Placement(transformation(extent={{74,72},{86,84}})));
  Interfaces.Adapters.TC TCl_back
    annotation (Placement(transformation(extent={{154,62},{166,74}})));
  Interfaces.Adapters.TS TSl_back
    annotation (Placement(transformation(extent={{74,62},{86,74}})));
  Interfaces.Adapters.TC TCl_pelvis
    annotation (Placement(transformation(extent={{154,52},{166,64}})));
  Interfaces.Adapters.TS TSl_pelvis
    annotation (Placement(transformation(extent={{74,52},{86,64}})));
  Interfaces.Adapters.TC TCl_lshoulder
    annotation (Placement(transformation(extent={{154,42},{166,54}})));
  Interfaces.Adapters.TS TSl_lshoulder
    annotation (Placement(transformation(extent={{74,42},{86,54}})));
  Interfaces.Adapters.TC TCl_rshoulder
    annotation (Placement(transformation(extent={{154,32},{166,44}})));
  Interfaces.Adapters.TS TSl_rshoulder
    annotation (Placement(transformation(extent={{74,32},{86,44}})));
  Interfaces.Adapters.TC TCl_larm
    annotation (Placement(transformation(extent={{154,22},{166,34}})));
  Interfaces.Adapters.TS TSl_larm
    annotation (Placement(transformation(extent={{74,22},{86,34}})));
  Interfaces.Adapters.TC TCl_rarm
    annotation (Placement(transformation(extent={{154,12},{166,24}})));
  Interfaces.Adapters.TS TSl_rarm
    annotation (Placement(transformation(extent={{74,12},{86,24}})));
  Interfaces.Adapters.TC TCl_lhand
    annotation (Placement(transformation(extent={{154,2},{166,14}})));
  Interfaces.Adapters.TS TSl_lhand
    annotation (Placement(transformation(extent={{74,2},{86,14}})));
  Interfaces.Adapters.TC TCl_rhand
    annotation (Placement(transformation(extent={{154,-8},{166,4}})));
  Interfaces.Adapters.TS TSl_rhand
    annotation (Placement(transformation(extent={{74,-8},{86,4}})));
  Interfaces.Adapters.TC TCl_lthigh
    annotation (Placement(transformation(extent={{154,-18},{166,-6}})));
  Interfaces.Adapters.TS TSl_lthigh
    annotation (Placement(transformation(extent={{74,-18},{86,-6}})));
  Interfaces.Adapters.TC TCl_rthigh
    annotation (Placement(transformation(extent={{154,-28},{166,-16}})));
  Interfaces.Adapters.TS TSl_rthigh
    annotation (Placement(transformation(extent={{74,-28},{86,-16}})));
  Interfaces.Adapters.TC TCl_lleg
    annotation (Placement(transformation(extent={{154,-38},{166,-26}})));
  Interfaces.Adapters.TS TSl_lleg
    annotation (Placement(transformation(extent={{74,-38},{86,-26}})));
  Interfaces.Adapters.TC TCl_rleg
    annotation (Placement(transformation(extent={{154,-48},{166,-36}})));
  Interfaces.Adapters.TS TSl_rleg
    annotation (Placement(transformation(extent={{74,-48},{86,-36}})));
  Interfaces.Adapters.TC TCl_lfoot
    annotation (Placement(transformation(extent={{154,-58},{166,-46}})));
  Interfaces.Adapters.TS TSl_lfoot
    annotation (Placement(transformation(extent={{74,-58},{86,-46}})));
  Interfaces.Adapters.TC TCl_rfoot
    annotation (Placement(transformation(extent={{154,-68},{166,-56}})));
  Interfaces.Adapters.TS TSl_rfoot
    annotation (Placement(transformation(extent={{74,-68},{86,-56}})));
  Interfaces.Adapters.TC TC_overall
    annotation (Placement(transformation(extent={{154,-78},{166,-66}})));
  Interfaces.Adapters.TS TS_overall
    annotation (Placement(transformation(extent={{74,-78},{86,-66}})));
  Interfaces.Adapters.TC TCl_neck
    annotation (Placement(transformation(extent={{154,82},{166,94}})));
  Interfaces.Adapters.TS TSl_neck
    annotation (Placement(transformation(extent={{74,82},{86,94}})));

  inner Interfaces.Parameters parameters
    annotation (Placement(transformation(extent={{-40,66},{-20,86}})));

  // Input table for standalone-mode
  Modelica.Blocks.Sources.CombiTimeTable InputData(
    tableOnFile=true,
    tableName="table",
    columns=2:132,
    fileName=ModelicaServices.ExternalReferences.loadResource(
        "modelica://NOODEL/DataBase/BC_Gagge_29-17-29_degree.txt")) if not parameters.UseCoupling
    annotation (Placement(transformation(extent={{20,66},{0,86}})));

// Input Data for coupling
// Test four humidity


// Variables for damping
  // Convection
  // Modelica.SIunits.HeatFlux Input_qcon_head_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_chest_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_back_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_pelvis_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_r_Shoulder_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_l_Shoulder_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_r_Arm_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_l_Arm_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_r_Hand_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_l_Hand_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_r_Thigh_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_l_Thigh_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_r_Leg_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_l_Leg_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_r_Foot_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qcon_l_Foot_damped(start=0) if parameters.UseCoupling;
  // //Radiation
  // Modelica.SIunits.HeatFlux Input_qrad_head_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_chest_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_back_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_pelvis_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_r_Shoulder_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_l_Shoulder_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_r_Arm_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_l_Arm_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_r_Hand_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_l_Hand_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_r_Thigh_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_l_Thigh_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_r_Leg_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_l_Leg_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_r_Foot_damped(start=0) if parameters.UseCoupling;
  // Modelica.SIunits.HeatFlux Input_qrad_l_Foot_damped(start=0) if parameters.UseCoupling;

  // Input
    // Convective heat flux
Modelica.Blocks.Interfaces.RealInput Input_qcon_head(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,80},{-220,90}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_chest(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,70},{-220,80}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_back(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,60},{-220,70}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_pelvis(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,50},{-220,60}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_r_Shoulder(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,40},{-220,50}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_l_Shoulder(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,30},{-220,40}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_r_Arm(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,20},{-220,30}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_l_Arm(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,10},{-220,20}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_r_Hand(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,0},{-220,10}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_l_Hand(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,-12},{-220,0}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_r_Thigh(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,-22},{-220,-12}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_l_Thigh(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,-32},{-220,-22}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_r_Leg(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,-42},{-220,-32}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_l_Leg(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,-52},{-220,-42}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_r_Foot(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,-62},{-220,-52}})));
Modelica.Blocks.Interfaces.RealInput Input_qcon_l_Foot(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-230,-72},{-220,-62}})));
    // Radiative heat flux
Modelica.Blocks.Interfaces.RealInput Input_qrad_head(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,82},{-134,92}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_chest(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,72},{-134,82}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_back(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,62},{-134,72}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_pelvis(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,52},{-134,62}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_r_Shoulder(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,42},{-134,52}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_l_Shoulder(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,32},{-134,42}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_r_Arm(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,22},{-134,32}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_l_Arm(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,12},{-134,22}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_r_Hand(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,2},{-134,12}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_l_Hand(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,-10},{-134,2}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_r_Thigh(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,-20},{-134,-10}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_l_Thigh(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,-30},{-134,-20}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_r_Leg(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,-40},{-134,-30}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_l_Leg(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,-50},{-134,-40}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_r_Foot(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,-60},{-134,-50}})));
Modelica.Blocks.Interfaces.RealInput Input_qrad_l_Foot(start=5, unit="W/m2")
    annotation (Placement(transformation(extent={{-144,-70},{-134,-60}})));







// Output Data for coupling

// Temperature of outer layer (includes clothing)
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_head(unit="K")
    annotation (Placement(transformation(extent={{266,80},{276,90}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_chest(unit="K")
    annotation (Placement(transformation(extent={{266,70},{276,80}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_back(unit="K")
    annotation (Placement(transformation(extent={{266,60},{276,70}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_pelvis(unit="K")
    annotation (Placement(transformation(extent={{266,50},{276,60}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_r_Shoulder(unit="K")
    annotation (Placement(transformation(extent={{266,40},{276,50}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_l_Shoulder(unit="K")
    annotation (Placement(transformation(extent={{266,30},{276,40}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_r_Arm(unit="K")
    annotation (Placement(transformation(extent={{266,20},{276,30}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_l_Arm(unit="K")
    annotation (Placement(transformation(extent={{266,10},{276,20}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_r_Hand(unit="K")
    annotation (Placement(transformation(extent={{266,0},{276,10}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_l_Hand(unit="K")
    annotation (Placement(transformation(extent={{266,-12},{276,0}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_r_Thigh(unit="K")
    annotation (Placement(transformation(extent={{266,-22},{276,-12}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_l_Thigh(unit="K")
    annotation (Placement(transformation(extent={{266,-32},{276,-22}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_r_Leg(unit="K")
    annotation (Placement(transformation(extent={{266,-42},{276,-32}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_l_Leg(unit="K")
    annotation (Placement(transformation(extent={{266,-52},{276,-42}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_r_Foot(unit="K")
    annotation (Placement(transformation(extent={{266,-62},{276,-52}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Outer_l_Foot(unit="K")
    annotation (Placement(transformation(extent={{266,-72},{276,-62}})));

    // Temperature of Skin layer
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_head(unit="K")
    annotation (Placement(transformation(extent={{354,80},{364,90}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_chest(unit="K")
    annotation (Placement(transformation(extent={{354,70},{364,80}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_back(unit="K")
    annotation (Placement(transformation(extent={{354,60},{364,70}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_pelvis(unit="K")
    annotation (Placement(transformation(extent={{354,50},{364,60}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_r_Shoulder(unit="K")
    annotation (Placement(transformation(extent={{354,40},{364,50}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_l_Shoulder(unit="K")
    annotation (Placement(transformation(extent={{354,30},{364,40}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_r_Arm(unit="K")
    annotation (Placement(transformation(extent={{354,20},{364,30}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_l_Arm(unit="K")
    annotation (Placement(transformation(extent={{354,10},{364,20}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_r_Hand(unit="K")
    annotation (Placement(transformation(extent={{354,0},{364,10}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_l_Hand(unit="K")
    annotation (Placement(transformation(extent={{354,-12},{364,0}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_r_Thigh(unit="K")
    annotation (Placement(transformation(extent={{354,-22},{364,-12}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_l_Thigh(unit="K")
    annotation (Placement(transformation(extent={{354,-32},{364,-22}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_r_Leg(unit="K")
    annotation (Placement(transformation(extent={{354,-42},{364,-32}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_l_Leg(unit="K")
    annotation (Placement(transformation(extent={{354,-52},{364,-42}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_r_Foot(unit="K")
    annotation (Placement(transformation(extent={{354,-62},{364,-52}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Skin_l_Foot(unit="K")
    annotation (Placement(transformation(extent={{354,-72},{364,-62}})));

    // Temperature of Core layer
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_head(unit="K")
    annotation (Placement(transformation(extent={{450,80},{460,90}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_chest(unit="K")
    annotation (Placement(transformation(extent={{450,70},{460,80}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_back(unit="K")
    annotation (Placement(transformation(extent={{450,60},{460,70}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_pelvis(unit="K")
    annotation (Placement(transformation(extent={{450,50},{460,60}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_r_Shoulder(unit="K")
    annotation (Placement(transformation(extent={{450,40},{460,50}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_l_Shoulder(unit="K")
    annotation (Placement(transformation(extent={{450,30},{460,40}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_r_Arm(unit="K")
    annotation (Placement(transformation(extent={{450,20},{460,30}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_l_Arm(unit="K")
    annotation (Placement(transformation(extent={{450,10},{460,20}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_r_Hand(unit="K")
    annotation (Placement(transformation(extent={{450,0},{460,10}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_l_Hand(unit="K")
    annotation (Placement(transformation(extent={{450,-12},{460,0}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_r_Thigh(unit="K")
    annotation (Placement(transformation(extent={{450,-22},{460,-12}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_l_Thigh(unit="K")
    annotation (Placement(transformation(extent={{450,-32},{460,-22}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_r_Leg(unit="K")
    annotation (Placement(transformation(extent={{450,-42},{460,-32}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_l_Leg(unit="K")
    annotation (Placement(transformation(extent={{450,-52},{460,-42}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_r_Foot(unit="K")
    annotation (Placement(transformation(extent={{450,-62},{460,-52}})));
Modelica.Blocks.Interfaces.RealOutput Output_T_Core_l_Foot(unit="K")
    annotation (Placement(transformation(extent={{450,-72},{460,-62}})));


// Thermal Sensation
 Modelica.Blocks.Interfaces.RealOutput Output_TS_overall
     annotation (Placement(transformation(extent={{106,88},{116,98}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_head
     annotation (Placement(transformation(extent={{106,78},{116,88}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_chest
     annotation (Placement(transformation(extent={{106,68},{116,78}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_back
     annotation (Placement(transformation(extent={{106,58},{116,68}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_pelvis
     annotation (Placement(transformation(extent={{106,48},{116,58}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_r_Shoulder
     annotation (Placement(transformation(extent={{106,38},{116,48}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_l_Shoulder
     annotation (Placement(transformation(extent={{106,28},{116,38}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_r_Arm
     annotation (Placement(transformation(extent={{106,18},{116,28}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_l_Arm
     annotation (Placement(transformation(extent={{106,8},{116,18}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_r_Hand
     annotation (Placement(transformation(extent={{106,-2},{116,8}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_l_Hand
     annotation (Placement(transformation(extent={{106,-14},{116,-2}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_r_Thigh
     annotation (Placement(transformation(extent={{106,-24},{116,-14}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_l_Thigh
     annotation (Placement(transformation(extent={{106,-34},{116,-24}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_r_Leg
     annotation (Placement(transformation(extent={{106,-44},{116,-34}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_l_Leg
     annotation (Placement(transformation(extent={{106,-54},{116,-44}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_r_Foot
     annotation (Placement(transformation(extent={{106,-64},{116,-54}})));
     Modelica.Blocks.Interfaces.RealOutput Output_TS_l_Foot
     annotation (Placement(transformation(extent={{106,-74},{116,-64}})));

    // Thermal Comfort
Modelica.Blocks.Interfaces.RealOutput Output_TC_overall
    annotation (Placement(transformation(extent={{184,88},{194,98}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_head
    annotation (Placement(transformation(extent={{184,78},{194,88}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_chest
    annotation (Placement(transformation(extent={{184,68},{194,78}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_back
    annotation (Placement(transformation(extent={{184,58},{194,68}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_pelvis
    annotation (Placement(transformation(extent={{184,48},{194,58}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_r_Shoulder
    annotation (Placement(transformation(extent={{184,38},{194,48}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_l_Shoulder
    annotation (Placement(transformation(extent={{184,28},{194,38}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_r_Arm
    annotation (Placement(transformation(extent={{184,18},{194,28}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_l_Arm
    annotation (Placement(transformation(extent={{184,8},{194,18}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_r_Hand
    annotation (Placement(transformation(extent={{184,-2},{194,8}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_l_Hand
    annotation (Placement(transformation(extent={{184,-14},{194,-2}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_r_Thigh
    annotation (Placement(transformation(extent={{184,-24},{194,-14}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_l_Thigh
    annotation (Placement(transformation(extent={{184,-34},{194,-24}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_r_Leg
    annotation (Placement(transformation(extent={{184,-44},{194,-34}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_l_Leg
    annotation (Placement(transformation(extent={{184,-54},{194,-44}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_r_Foot
    annotation (Placement(transformation(extent={{184,-64},{194,-54}})));
Modelica.Blocks.Interfaces.RealOutput Output_TC_l_Foot
    annotation (Placement(transformation(extent={{184,-74},{194,-64}})));


  Interfaces.Adapters.TS TSl_head1
    annotation (Placement(transformation(extent={{318,92},{330,104}})));
  Interfaces.Adapters.TS TSl_chest1
    annotation (Placement(transformation(extent={{318,72},{330,84}})));
  Interfaces.Adapters.TS TSl_back1
    annotation (Placement(transformation(extent={{318,62},{330,74}})));
  Interfaces.Adapters.TS TSl_pelvis1
    annotation (Placement(transformation(extent={{318,52},{330,64}})));
  Interfaces.Adapters.TS TSl_lshoulder1
    annotation (Placement(transformation(extent={{318,42},{330,54}})));
  Interfaces.Adapters.TS TSl_rshoulder1
    annotation (Placement(transformation(extent={{318,32},{330,44}})));
  Interfaces.Adapters.TS TSl_larm1
    annotation (Placement(transformation(extent={{318,22},{330,34}})));
  Interfaces.Adapters.TS TSl_rarm1
    annotation (Placement(transformation(extent={{318,12},{330,24}})));
  Interfaces.Adapters.TS TSl_lhand1
    annotation (Placement(transformation(extent={{318,2},{330,14}})));
  Interfaces.Adapters.TS TSl_rhand1
    annotation (Placement(transformation(extent={{318,-8},{330,4}})));
  Interfaces.Adapters.TS TSl_lthigh1
    annotation (Placement(transformation(extent={{318,-18},{330,-6}})));
  Interfaces.Adapters.TS TSl_rthigh1
    annotation (Placement(transformation(extent={{318,-28},{330,-16}})));
  Interfaces.Adapters.TS TSl_lleg1
    annotation (Placement(transformation(extent={{318,-38},{330,-26}})));
  Interfaces.Adapters.TS TSl_rleg1
    annotation (Placement(transformation(extent={{318,-48},{330,-36}})));
  Interfaces.Adapters.TS TSl_lfoot1
    annotation (Placement(transformation(extent={{318,-58},{330,-46}})));
  Interfaces.Adapters.TS TSl_rfoot1
    annotation (Placement(transformation(extent={{318,-68},{330,-56}})));
  Interfaces.Adapters.TS TS_overall1
    annotation (Placement(transformation(extent={{318,-78},{330,-66}})));
  Interfaces.Adapters.TS TSl_neck1
    annotation (Placement(transformation(extent={{318,82},{330,94}})));
  Interfaces.Adapters.TS TSl_head2
    annotation (Placement(transformation(extent={{236,92},{248,104}})));
  Interfaces.Adapters.TS TSl_chest2
    annotation (Placement(transformation(extent={{236,72},{248,84}})));
  Interfaces.Adapters.TS TSl_back2
    annotation (Placement(transformation(extent={{236,62},{248,74}})));
  Interfaces.Adapters.TS TSl_pelvis2
    annotation (Placement(transformation(extent={{236,52},{248,64}})));
  Interfaces.Adapters.TS TSl_lshoulder2
    annotation (Placement(transformation(extent={{236,42},{248,54}})));
  Interfaces.Adapters.TS TSl_rshoulder2
    annotation (Placement(transformation(extent={{236,32},{248,44}})));
  Interfaces.Adapters.TS TSl_larm2
    annotation (Placement(transformation(extent={{236,22},{248,34}})));
  Interfaces.Adapters.TS TSl_rarm2
    annotation (Placement(transformation(extent={{236,12},{248,24}})));
  Interfaces.Adapters.TS TSl_lhand2
    annotation (Placement(transformation(extent={{236,2},{248,14}})));
  Interfaces.Adapters.TS TSl_rhand2
    annotation (Placement(transformation(extent={{236,-8},{248,4}})));
  Interfaces.Adapters.TS TSl_lthigh2
    annotation (Placement(transformation(extent={{236,-18},{248,-6}})));
  Interfaces.Adapters.TS TSl_rthigh2
    annotation (Placement(transformation(extent={{236,-28},{248,-16}})));
  Interfaces.Adapters.TS TSl_lleg2
    annotation (Placement(transformation(extent={{236,-38},{248,-26}})));
  Interfaces.Adapters.TS TSl_rleg2
    annotation (Placement(transformation(extent={{236,-48},{248,-36}})));
  Interfaces.Adapters.TS TSl_lfoot2
    annotation (Placement(transformation(extent={{236,-58},{248,-46}})));
  Interfaces.Adapters.TS TSl_rfoot2
    annotation (Placement(transformation(extent={{236,-68},{248,-56}})));
  Interfaces.Adapters.TS TS_overall2
    annotation (Placement(transformation(extent={{236,-78},{248,-66}})));
  Interfaces.Adapters.TS TSl_neck2
    annotation (Placement(transformation(extent={{236,82},{248,94}})));
  Interfaces.Adapters.TS TSl_head3
    annotation (Placement(transformation(extent={{412,92},{424,104}})));
  Interfaces.Adapters.TS TSl_chest3
    annotation (Placement(transformation(extent={{412,72},{424,84}})));
  Interfaces.Adapters.TS TSl_back3
    annotation (Placement(transformation(extent={{412,62},{424,74}})));
  Interfaces.Adapters.TS TSl_pelvis3
    annotation (Placement(transformation(extent={{412,52},{424,64}})));
  Interfaces.Adapters.TS TSl_lshoulder3
    annotation (Placement(transformation(extent={{412,42},{424,54}})));
  Interfaces.Adapters.TS TSl_rshoulder3
    annotation (Placement(transformation(extent={{412,32},{424,44}})));
  Interfaces.Adapters.TS TSl_larm3
    annotation (Placement(transformation(extent={{412,22},{424,34}})));
  Interfaces.Adapters.TS TSl_rarm3
    annotation (Placement(transformation(extent={{412,12},{424,24}})));
  Interfaces.Adapters.TS TSl_lhand3
    annotation (Placement(transformation(extent={{412,2},{424,14}})));
  Interfaces.Adapters.TS TSl_rhand3
    annotation (Placement(transformation(extent={{412,-8},{424,4}})));
  Interfaces.Adapters.TS TSl_lthigh3
    annotation (Placement(transformation(extent={{412,-18},{424,-6}})));
  Interfaces.Adapters.TS TSl_rthigh3
    annotation (Placement(transformation(extent={{412,-28},{424,-16}})));
  Interfaces.Adapters.TS TSl_lleg3
    annotation (Placement(transformation(extent={{412,-38},{424,-26}})));
  Interfaces.Adapters.TS TSl_rleg3
    annotation (Placement(transformation(extent={{412,-48},{424,-36}})));
  Interfaces.Adapters.TS TSl_lfoot3
    annotation (Placement(transformation(extent={{412,-58},{424,-46}})));
  Interfaces.Adapters.TS TSl_rfoot3
    annotation (Placement(transformation(extent={{412,-68},{424,-56}})));
  Interfaces.Adapters.TS TS_overall3
    annotation (Placement(transformation(extent={{412,-78},{424,-66}})));
  Interfaces.Adapters.TS TSl_neck3
    annotation (Placement(transformation(extent={{412,82},{424,94}})));
algorithm

  // Mean thermal sensation and mean thermal comfort
  TSmean := (head.TSl + chest.TSl+ back.TSl + pelvis.TSl+ l_Shoulder.TSl+
    r_Shoulder.TSl+ l_Arm.TSl+ r_Arm.TSl+ l_Hand.TSl+ r_Hand.TSl+ l_Thigh.TSl
     + r_Thigh.TSl+ l_Leg.TSl+r_Leg.TSl+ l_Foot.TSl+ r_Foot.TSl)/16;

  TCmean := (head.TCl*head.skin.S + chest.TCl*chest.skin.S + back.TCl*
    back.skin.S + pelvis.TCl*pelvis.skin.S + l_Shoulder.TCl*l_Shoulder.skin.S +
    r_Shoulder.TCl*r_Shoulder.skin.S + l_Arm.TCl*l_Arm.skin.S + r_Arm.TCl*r_Arm.skin.S
     + l_Hand.TCl*l_Hand.skin.S + r_Hand.TCl*r_Hand.skin.S + l_Thigh.TCl*
    l_Thigh.skin.S + r_Thigh.TCl*r_Thigh.skin.S + l_Leg.TCl*l_Leg.skin.S +
    r_Leg.TCl*r_Leg.skin.S + l_Foot.TCl*l_Foot.skin.S + r_Foot.TCl*r_Foot.skin.S)
    /Stotal;

  // Average temperatures
  TskinAve := (head.skin.T*head.skin.S+chest.skin.T*chest.skin.S+back.skin.T*back.skin.S+pelvis.skin.T*pelvis.skin.S+l_Shoulder.skin.T*l_Shoulder.skin.S
            +r_Shoulder.skin.T*r_Shoulder.skin.S+l_Arm.skin.T*l_Arm.skin.S+r_Arm.skin.T*r_Arm.skin.S+l_Hand.skin.T*l_Hand.skin.S+r_Hand.skin.T*r_Hand.skin.S
            +l_Thigh.skin.T*l_Thigh.skin.S+r_Thigh.skin.T*r_Thigh.skin.S+l_Leg.skin.T*l_Leg.skin.S+r_Leg.skin.T*r_Leg.skin.S+l_Foot.skin.T*l_Foot.skin.S
            +r_Foot.skin.T*r_Foot.skin.S)/Stotal;
            // Tskin ave seven point method according Hardy and DuBois
  TskinAve_seven_point := 0.07*head.skin.T+0.35*chest.skin.T+0.14*(l_Arm.skin.T+r_Arm.skin.T)/2+0.05*(l_Hand.skin.T+r_Hand.skin.T)/2+0.19*(l_Thigh.skin.T+r_Thigh.skin.T)/2
                          +0.13*(l_Leg.skin.T+r_Leg.skin.T)/2+0.07*(l_Foot.skin.T+r_Foot.skin.T)/2;

  TsetpsyAve := (head.skin.TSETpsy*head.skin.S+chest.skin.TSETpsy*chest.skin.S+back.skin.TSETpsy*back.skin.S+pelvis.skin.TSETpsy*pelvis.skin.S+l_Shoulder.skin.TSETpsy*l_Shoulder.skin.S
            +r_Shoulder.skin.TSETpsy*r_Shoulder.skin.S+l_Arm.skin.TSETpsy*l_Arm.skin.S+r_Arm.skin.TSETpsy*r_Arm.skin.S+l_Hand.skin.TSETpsy*l_Hand.skin.S+r_Hand.skin.TSETpsy*r_Hand.skin.S
            +l_Thigh.skin.TSETpsy*l_Thigh.skin.S+r_Thigh.skin.TSETpsy*r_Thigh.skin.S+l_Leg.skin.TSETpsy*l_Leg.skin.S+r_Leg.skin.TSETpsy*r_Leg.skin.S+l_Foot.skin.TSETpsy*l_Foot.skin.S
            +r_Foot.skin.TSETpsy*r_Foot.skin.S)/Stotal;

  TsetAve := (head.skin.TSET*head.skin.S+chest.skin.TSET*chest.skin.S+back.skin.TSET*back.skin.S+pelvis.skin.TSET*pelvis.skin.S+l_Shoulder.skin.TSET*l_Shoulder.skin.S
            +r_Shoulder.skin.TSET*r_Shoulder.skin.S+l_Arm.skin.TSET*l_Arm.skin.S+r_Arm.skin.TSET*r_Arm.skin.S+l_Hand.skin.TSET*l_Hand.skin.S+r_Hand.skin.TSET*r_Hand.skin.S
            +l_Thigh.skin.TSET*l_Thigh.skin.S+r_Thigh.skin.TSET*r_Thigh.skin.S+l_Leg.skin.TSET*l_Leg.skin.S+r_Leg.skin.TSET*r_Leg.skin.S+l_Foot.skin.TSET*l_Foot.skin.S
            +r_Foot.skin.TSET*r_Foot.skin.S)/Stotal;

  // Heat fluxes
  Qe := (head.skin.E+chest.skin.E+back.skin.E+pelvis.skin.E+l_Shoulder.skin.E+r_Shoulder.skin.E+l_Arm.skin.E+r_Arm.skin.E+
  l_Hand.skin.E+r_Hand.skin.E+l_Thigh.skin.E+r_Thigh.skin.E+l_Leg.skin.E+r_Leg.skin.E+l_Foot.skin.E+r_Foot.skin.E);

  Qmet:=(head.core.Q + chest.core.Q + back.core.Q + pelvis.core.Q + l_Shoulder.core.Q
     + r_Shoulder.core.Q + l_Arm.core.Q + r_Arm.core.Q + l_Hand.core.Q + r_Hand.core.Q
     + l_Thigh.core.Q + r_Thigh.core.Q + l_Leg.core.Q + r_Leg.core.Q + l_Foot.core.Q
     + r_Foot.core.Q+head.skin.Q + chest.skin.Q + back.skin.Q + pelvis.skin.Q + l_Shoulder.skin.Q
     + r_Shoulder.skin.Q + l_Arm.skin.Q + r_Arm.skin.Q + l_Hand.skin.Q + r_Hand.skin.Q
     + l_Thigh.skin.Q + r_Thigh.skin.Q + l_Leg.skin.Q + r_Leg.skin.Q + l_Foot.skin.Q
     + r_Foot.skin.Q);

  Qshiv:=(head.core.CH + chest.core.CH + back.core.CH + pelvis.core.CH +
    l_Shoulder.core.CH + r_Shoulder.core.CH + l_Arm.core.CH + r_Arm.core.CH +
    l_Hand.core.CH + r_Hand.core.CH + l_Thigh.core.CH + r_Thigh.core.CH + l_Leg.core.CH
     + r_Leg.core.CH + l_Foot.core.CH + r_Foot.core.CH);

public
  Components.Segments.Head head annotation (Placement(transformation(
          extent={{-38,-6},{20,32}},rotation=0)));
  Components.Segments.Chest chest annotation (Placement(transformation(
          extent={{-40,-16},{2,12}},  rotation=0)));
  Components.Compartments.CentralBlood centralBlood
    annotation (Placement(transformation(extent={{-16,-24},{-6,-14}},
                                                                   rotation=
           0)));
  Components.Segments.Back back annotation (Placement(transformation(
          extent={{-26,-18},{22,14}},
                                    rotation=0)));
  Components.Segments.Pelvis pelvis annotation (Placement(
        transformation(extent={{-34,-46},{12,-18}},
                                                 rotation=0)));
  Components.Segments.R_Shoulder r_Shoulder annotation (Placement(
        transformation(extent={{-50,-18},{-10,12}},rotation=0)));
  Components.Segments.L_Shoulder l_Shoulder
    annotation (Placement(transformation(extent={{-8,-18},{28,8}},rotation=0)));
  Components.Segments.R_Arm r_Arm annotation (Placement(transformation(
          extent={{-48,-30},{-16,-2}},
                                     rotation=0)));
  Components.Segments.R_Hand r_Hand annotation (Placement(
        transformation(extent={{-50,-42},{-16,-12}},
                                                   rotation=0)));
  Components.Segments.R_Thigh r_Thigh annotation (Placement(
        transformation(extent={{-30,-64},{-4,-28}}, rotation=0)));
  Components.Segments.L_Hand l_Hand annotation (Placement(
        transformation(extent={{-6,-44},{28,-16}},
                                                 rotation=0)));
  Components.Segments.L_Thigh l_Thigh annotation (Placement(
        transformation(extent={{-18,-60},{6,-32}}, rotation=0)));
  Components.Segments.R_Leg r_Leg annotation (Placement(transformation(
          extent={{-32,-76},{-4,-42}}, rotation=0)));
  Components.Segments.L_Leg l_Leg annotation (Placement(transformation(
          extent={{-24,-74},{18,-42}},rotation=0)));
  Components.Segments.R_Foot r_Foot annotation (Placement(
        transformation(extent={{-38,-84},{0,-58}},  rotation=0)));
  Components.Segments.L_Foot l_Foot annotation (Placement(
        transformation(extent={{-18,-84},{16,-56}}, rotation=0)));

  Components.Segments.L_Arm l_Arm
    annotation (Placement(transformation(extent={{-8,-32},{28,-4}},
                                                                  rotation=0)));
equation
  connect(head.bloodTherm, centralBlood.bloodTherm1) annotation (Line(
        points={{-9,7.68},{-9,-14.5},{-9,-14.5}},
                                            color={255,186,151},
      pattern=LinePattern.Dot));
  connect(back.bloodTherm, centralBlood.bloodTherm1) annotation (Line(
        points={{-2,-6.48},{-2,-14.5},{-9,-14.5}},
                                        color={255,186,151},
      pattern=LinePattern.Dot));
  connect(chest.bloodTherm, centralBlood.bloodTherm) annotation (Line(
        points={{-19,-5.92},{-19,-14.5},{-13,-14.5}},
                                             color={255,186,151},
      pattern=LinePattern.Dot));
  connect(l_Shoulder.bloodTherm, centralBlood.bloodTherm6) annotation (Line(
        points={{10,-8.64},{14,-8.64},{14,-16},{4,-16},{4,-17.1},{-6.5,-17.1}},
                                                      color={255,186,151},
      pattern=LinePattern.Dot));
  connect(r_Shoulder.bloodTherm, centralBlood.bloodTherm4) annotation (Line(
        points={{-30,-7.2},{-30,-17},{-15.5,-17}},
                                             color={255,186,151},
      pattern=LinePattern.Dot));
  connect(pelvis.bloodTherm, centralBlood.bloodTherm3) annotation (Line(
        points={{-11,-35.92},{-11,-23.5},{-9,-23.5}},
                                           color={255,186,151},
      pattern=LinePattern.Dot));
  connect(l_Foot.bloodTherm, centralBlood.bloodTherm3) annotation (Line(
        points={{-1,-73.92},{18,-73.92},{18,-23.5},{-9,-23.5}},
                                                 color={255,186,151},
      pattern=LinePattern.Dot));
  connect(r_Foot.bloodTherm, centralBlood.bloodTherm2) annotation (Line(
        points={{-19,-74.64},{-28,-74.64},{-28,-24},{-13,-24},{-13,-23.5}},
                                                              color={255,186,151},
      pattern=LinePattern.Dot));

  // Thermal sensation and comfort
  TSlo = {head.TSl,chest.TSl,back.TSl,pelvis.TSl,l_Shoulder.TSl,r_Shoulder.TSl,l_Arm.TSl,r_Arm.TSl,l_Hand.TSl,
      r_Hand.TSl,l_Thigh.TSl,r_Thigh.TSl,l_Leg.TSl,r_Leg.TSl,l_Foot.TSl,r_Foot.TSl};
  TClo = {head.TCl,chest.TCl,back.TCl,pelvis.TCl,l_Shoulder.TCl,r_Shoulder.TCl,l_Arm.TCl,r_Arm.TCl,l_Hand.TCl,
      r_Hand.TCl,l_Thigh.TCl,r_Thigh.TCl,l_Leg.TCl,r_Leg.TCl,l_Foot.TCl,r_Foot.TCl};

  TSo = min(max(NOODEL.Functions.TSo(TSlo), -3), 3);
  TCo = min(max(NOODEL.Functions.TCo(TClo), -3), 3);

  //Integrated warm and cold signal as control variables
  Cerr=head.core.twoThermCompartment.therm1.T-36.01;
  Cwarm=head.core.WARM;
  Ccold=head.core.COLD;

  WRMS = head.SKINR*head.skin.WARM+chest.SKINR*chest.skin.WARM+back.SKINR*back.skin.WARM
        +pelvis.SKINR*pelvis.skin.WARM+l_Shoulder.SKINR*l_Shoulder.skin.WARM+r_Shoulder.SKINR*r_Shoulder.skin.WARM
        +l_Arm.SKINR*l_Arm.skin.WARM+r_Arm.SKINR*r_Arm.skin.WARM+l_Thigh.SKINR*l_Thigh.skin.WARM
        +r_Thigh.SKINR*r_Thigh.skin.WARM+l_Leg.SKINR*l_Leg.skin.WARM +r_Leg.SKINR*r_Leg.skin.WARM
        +l_Foot.SKINR*l_Foot.skin.WARM+r_Foot.SKINR*r_Foot.skin.WARM+l_Hand.SKINR*l_Hand.skin.WARM
        +r_Hand.SKINR*r_Hand.skin.WARM;

  CLDS = head.SKINR*head.skin.COLD+chest.SKINR*chest.skin.COLD+back.SKINR*back.skin.COLD
         +pelvis.SKINR*pelvis.skin.COLD+l_Shoulder.SKINR*l_Shoulder.skin.COLD+r_Shoulder.SKINR*r_Shoulder.skin.COLD
         +l_Arm.SKINR*l_Arm.skin.COLD+r_Arm.SKINR*r_Arm.skin.COLD+l_Thigh.SKINR*l_Thigh.skin.COLD
         +r_Thigh.SKINR*r_Thigh.skin.COLD+l_Leg.SKINR*l_Leg.skin.COLD+r_Leg.SKINR*r_Leg.skin.COLD
         +l_Foot.SKINR*l_Foot.skin.COLD+r_Foot.SKINR*r_Foot.skin.COLD+l_Hand.SKINR*l_Hand.skin.COLD
         +r_Hand.SKINR*r_Hand.skin.COLD;

  // Evaporative heat transfer
  ESW_total = head.skin.ESW+chest.skin.ESW+back.skin.ESW+pelvis.skin.ESW+l_Shoulder.skin.ESW+r_Shoulder.skin.ESW+l_Arm.skin.ESW
            +r_Arm.skin.ESW+l_Hand.skin.ESW+r_Hand.skin.ESW+l_Thigh.skin.ESW+r_Thigh.skin.ESW+l_Leg.skin.ESW+r_Leg.skin.ESW
            +l_Foot.skin.ESW+r_Foot.skin.ESW;

  E_total_m2 = (head.skin.E+back.skin.E+chest.skin.E+pelvis.skin.E
          +l_Shoulder.skin.E+r_Shoulder.skin.E+l_Arm.skin.E
          +r_Arm.skin.E+l_Thigh.skin.E+r_Thigh.skin.E+l_Leg.skin.E
          +r_Leg.skin.E+r_Foot.skin.E+l_Foot.skin.E+l_Hand.skin.E
          +r_Hand.skin.E)/Stotal;

  //Signals for vasodilation and vasoconstriction
  ST = max(-parameters.CST*head.core.ERROR-parameters.SST*(WRMS-CLDS)+parameters.PST*head.core.COLD*CLDS,0);
  DL = max(parameters.CDL*head.core.ERROR+parameters.SDL*(WRMS-CLDS)+parameters.PDL*head.core.WARM*WRMS,0);

  // Setting boundary conditions depending on mode (Standalone or Coupling)
  if parameters.UseCoupling then
    for k in 1:16 loop
      parameters.Tair[k] = 35;
      parameters.Trad[k] = 35;
      parameters.v[k] = 0.15;
      parameters.Icl[k] = parameters.Icl_Coupled[k];
      parameters.CSF[k] = parameters.CSF_Coupled[k];
      parameters.Tmt[k] = parameters.Tmt_Coupled[k];
      parameters.CSR[k] = parameters.CSR_Coupled[k];
      parameters.ISolar[k] = parameters.ISolar_Coupled[k];
    end for;
     parameters.rh = parameters.rh_Coupled;
     parameters.met = parameters.met_Coupled;
     parameters.Position = parameters.Position_Coupled;
  else
   for k in 1:16 loop
      connect(InputData.y[k], parameters.Tair[k]);
       connect(InputData.y[k+16], parameters.Trad[k]);
       connect(InputData.y[k+32], parameters.v[k]);
       connect(InputData.y[k+50], parameters.Icl[k]);
       connect(InputData.y[k+66], parameters.CSF[k]);
       connect(InputData.y[k+82], parameters.Tmt[k]);
       connect(InputData.y[k+98], parameters.CSR[k]);
       connect(InputData.y[k+114], parameters.ISolar[k]);
    end for;
      connect(InputData.y[49], parameters.rh);
      connect(InputData.y[50], parameters.met);
      connect(InputData.y[131], parameters.Position);
  end if;

 // variable start values for skin and core temperature
  head.skin.T0skin = parameters.T0sk[1];
  chest.skin.T0skin = parameters.T0sk[2];
  back.skin.T0skin = parameters.T0sk[3];
  pelvis.skin.T0skin = parameters.T0sk[4];
  r_Shoulder.skin.T0skin = parameters.T0sk[5];
  l_Shoulder.skin.T0skin = parameters.T0sk[6];
  r_Arm.skin.T0skin = parameters.T0sk[7];
  l_Arm.skin.T0skin = parameters.T0sk[8];
  r_Hand.skin.T0skin = parameters.T0sk[9];
  l_Hand.skin.T0skin = parameters.T0sk[10];
  r_Thigh.skin.T0skin = parameters.T0sk[11];
  l_Thigh.skin.T0skin = parameters.T0sk[12];
  r_Leg.skin.T0skin = parameters.T0sk[13];
  l_Leg.skin.T0skin = parameters.T0sk[14];
  r_Foot.skin.T0skin = parameters.T0sk[15];
  l_Foot.skin.T0skin = parameters.T0sk[16];

  head.core.T0core = parameters.T0cr[1];
  chest.core.T0core = parameters.T0cr[2];
  back.core.T0core = parameters.T0cr[3];
  pelvis.core.T0core = parameters.T0cr[4];
  r_Shoulder.core.T0core = parameters.T0cr[5];
  l_Shoulder.core.T0core = parameters.T0cr[6];
  r_Arm.core.T0core = parameters.T0cr[7];
  l_Arm.core.T0core = parameters.T0cr[8];
  r_Hand.core.T0core = parameters.T0cr[9];
  l_Hand.core.T0core = parameters.T0cr[10];
  r_Thigh.core.T0core = parameters.T0cr[11];
  l_Thigh.core.T0core = parameters.T0cr[12];
  r_Leg.core.T0core = parameters.T0cr[13];
  l_Leg.core.T0core = parameters.T0cr[14];
  r_Foot.core.T0core = parameters.T0cr[15];
  l_Foot.core.T0core = parameters.T0cr[16];



  parameters.Tair[1] = head.skin.Tair;
  parameters.Tair[2] = chest.skin.Tair;
  parameters.Tair[3] = back.skin.Tair;
  parameters.Tair[4] = pelvis.skin.Tair;
  parameters.Tair[5] = r_Shoulder.skin.Tair;
  parameters.Tair[6] = l_Shoulder.skin.Tair;
  parameters.Tair[7] = r_Arm.skin.Tair;
  parameters.Tair[8] = l_Arm.skin.Tair;
  parameters.Tair[9] = r_Hand.skin.Tair;
  parameters.Tair[10] = l_Hand.skin.Tair;
  parameters.Tair[11] = r_Thigh.skin.Tair;
  parameters.Tair[12] = l_Thigh.skin.Tair;
  parameters.Tair[13] = r_Leg.skin.Tair;
  parameters.Tair[14] = l_Leg.skin.Tair;
  parameters.Tair[15] = r_Foot.skin.Tair;
  parameters.Tair[16] = l_Foot.skin.Tair;

  parameters.Trad[1] = head.skin.Trad;
  parameters.Trad[2] = chest.skin.Trad;
  parameters.Trad[3] = back.skin.Trad;
  parameters.Trad[4] = pelvis.skin.Trad;
  parameters.Trad[5] = r_Shoulder.skin.Trad;
  parameters.Trad[6] = l_Shoulder.skin.Trad;
  parameters.Trad[7] = r_Arm.skin.Trad;
  parameters.Trad[8] = l_Arm.skin.Trad;
  parameters.Trad[9] = r_Hand.skin.Trad;
  parameters.Trad[10] = l_Hand.skin.Trad;
  parameters.Trad[11] = r_Thigh.skin.Trad;
  parameters.Trad[12] = l_Thigh.skin.Trad;
  parameters.Trad[13] = r_Leg.skin.Trad;
  parameters.Trad[14] = l_Leg.skin.Trad;
  parameters.Trad[15] = r_Foot.skin.Trad;
  parameters.Trad[16] = l_Foot.skin.Trad;

  parameters.v[1] = head.skin.v;
  parameters.v[2] = chest.skin.v;
  parameters.v[3] = back.skin.v;
  parameters.v[4] = pelvis.skin.v;
  parameters.v[5] = r_Shoulder.skin.v;
  parameters.v[6] = l_Shoulder.skin.v;
  parameters.v[7] = r_Arm.skin.v;
  parameters.v[8] = l_Arm.skin.v;
  parameters.v[9] = r_Hand.skin.v;
  parameters.v[10] = l_Hand.skin.v;
  parameters.v[11] = r_Thigh.skin.v;
  parameters.v[12] = l_Thigh.skin.v;
  parameters.v[13] = r_Leg.skin.v;
  parameters.v[14] = l_Leg.skin.v;
  parameters.v[15] = r_Foot.skin.v;
  parameters.v[16] = l_Foot.skin.v;

  head.skin.Icl = parameters.Icl[1];
  chest.skin.Icl = parameters.Icl[2];
  back.skin.Icl = parameters.Icl[3];
  pelvis.skin.Icl = parameters.Icl[4];
  r_Shoulder.skin.Icl = parameters.Icl[5];
  l_Shoulder.skin.Icl = parameters.Icl[6];
  r_Arm.skin.Icl = parameters.Icl[7];
  l_Arm.skin.Icl = parameters.Icl[8];
  r_Hand.skin.Icl = parameters.Icl[9];
  l_Hand.skin.Icl = parameters.Icl[10];
  r_Thigh.skin.Icl = parameters.Icl[11];
  l_Thigh.skin.Icl = parameters.Icl[12];
  r_Leg.skin.Icl = parameters.Icl[13];
  l_Leg.skin.Icl = parameters.Icl[14];
  r_Foot.skin.Icl = parameters.Icl[15];
  l_Foot.skin.Icl = parameters.Icl[16];

  head.skin.CSF = parameters.CSF[1];
  chest.skin.CSF = parameters.CSF[2];
  back.skin.CSF = parameters.CSF[3];
  pelvis.skin.CSF = parameters.CSF[4];
  r_Shoulder.skin.CSF = parameters.CSF[5];
  l_Shoulder.skin.CSF = parameters.CSF[6];
  r_Arm.skin.CSF = parameters.CSF[7];
  l_Arm.skin.CSF = parameters.CSF[8];
  r_Hand.skin.CSF = parameters.CSF[9];
  l_Hand.skin.CSF = parameters.CSF[10];
  r_Thigh.skin.CSF = parameters.CSF[11];
  l_Thigh.skin.CSF = parameters.CSF[12];
  r_Leg.skin.CSF = parameters.CSF[13];
  l_Leg.skin.CSF = parameters.CSF[14];
  r_Foot.skin.CSF = parameters.CSF[15];
  l_Foot.skin.CSF = parameters.CSF[16];

  head.skin.CSR = parameters.CSR[1];
  chest.skin.CSR = parameters.CSR[2];
  back.skin.CSR = parameters.CSR[3];
  pelvis.skin.CSR = parameters.CSR[4];
  r_Shoulder.skin.CSR = parameters.CSR[5];
  l_Shoulder.skin.CSR = parameters.CSR[6];
  r_Arm.skin.CSR = parameters.CSR[7];
  l_Arm.skin.CSR = parameters.CSR[8];
  r_Hand.skin.CSR = parameters.CSR[9];
  l_Hand.skin.CSR = parameters.CSR[10];
  r_Thigh.skin.CSR = parameters.CSR[11];
  l_Thigh.skin.CSR = parameters.CSR[12];
  r_Leg.skin.CSR = parameters.CSR[13];
  l_Leg.skin.CSR = parameters.CSR[14];
  r_Foot.skin.CSR = parameters.CSR[15];
  l_Foot.skin.CSR = parameters.CSR[16];

  head.skin.Tmt = parameters.Tmt[1];
  chest.skin.Tmt = parameters.Tmt[2];
  back.skin.Tmt = parameters.Tmt[3];
  pelvis.skin.Tmt = parameters.Tmt[4];
  r_Shoulder.skin.Tmt = parameters.Tmt[5];
  l_Shoulder.skin.Tmt = parameters.Tmt[6];
  r_Arm.skin.Tmt = parameters.Tmt[7];
  l_Arm.skin.Tmt = parameters.Tmt[8];
  r_Hand.skin.Tmt = parameters.Tmt[9];
  l_Hand.skin.Tmt = parameters.Tmt[10];
  r_Thigh.skin.Tmt = parameters.Tmt[11];
  l_Thigh.skin.Tmt = parameters.Tmt[12];
  r_Leg.skin.Tmt = parameters.Tmt[13];
  l_Leg.skin.Tmt = parameters.Tmt[14];
  r_Foot.skin.Tmt = parameters.Tmt[15];
  l_Foot.skin.Tmt = parameters.Tmt[16];

  head.skin.ISolar = parameters.ISolar[1];
  chest.skin.ISolar = parameters.ISolar[2];
  back.skin.ISolar = parameters.ISolar[3];
  pelvis.skin.ISolar = parameters.ISolar[4];
  r_Shoulder.skin.ISolar = parameters.ISolar[5];
  l_Shoulder.skin.ISolar = parameters.ISolar[6];
  r_Arm.skin.ISolar = parameters.ISolar[7];
  l_Arm.skin.ISolar = parameters.ISolar[8];
  r_Hand.skin.ISolar = parameters.ISolar[9];
  l_Hand.skin.ISolar = parameters.ISolar[10];
  r_Thigh.skin.ISolar = parameters.ISolar[11];
  l_Thigh.skin.ISolar = parameters.ISolar[12];
  r_Leg.skin.ISolar = parameters.ISolar[13];
  l_Leg.skin.ISolar = parameters.ISolar[14];
  r_Foot.skin.ISolar = parameters.ISolar[15];
  l_Foot.skin.ISolar = parameters.ISolar[16];

  PAIR = parameters.rh*NOODEL.Functions.Ps(parameters.TempEnv);
  met = parameters.met;

  // Results for thermal sensation and comfort
  TSl_head.TSl=head.TSl;
  TSl_neck.TSl=head.TSl;
  TSl_chest.TSl=chest.TSl;
  TSl_back.TSl=back.TSl;
  TSl_pelvis.TSl=pelvis.TSl;
  TSl_lshoulder.TSl=l_Shoulder.TSl;
  TSl_rshoulder.TSl=r_Shoulder.TSl;
  TSl_larm.TSl=l_Arm.TSl;
  TSl_rarm.TSl=r_Arm.TSl;
  TSl_lhand.TSl=l_Hand.TSl;
  TSl_rhand.TSl=r_Hand.TSl;
  TSl_lthigh.TSl=l_Thigh.TSl;
  TSl_rthigh.TSl=r_Thigh.TSl;
  TSl_lleg.TSl=l_Leg.TSl;
  TSl_rleg.TSl=r_Leg.TSl;
  TSl_lfoot.TSl=l_Foot.TSl;
  TSl_rfoot.TSl=r_Foot.TSl;
  TS_overall.TSl=TSo;

  TCl_head.TCl=head.TCl;
  TCl_neck.TCl=head.TCl;
  TCl_chest.TCl=chest.TCl;
  TCl_back.TCl=back.TCl;
  TCl_pelvis.TCl=pelvis.TCl;
  TCl_lshoulder.TCl=l_Shoulder.TCl;
  TCl_rshoulder.TCl=r_Shoulder.TCl;
  TCl_larm.TCl=l_Arm.TCl;
  TCl_rarm.TCl=r_Arm.TCl;
  TCl_lhand.TCl=l_Hand.TCl;
  TCl_rhand.TCl=r_Hand.TCl;
  TCl_lthigh.TCl=l_Thigh.TCl;
  TCl_rthigh.TCl=r_Thigh.TCl;
  TCl_lleg.TCl=l_Leg.TCl;
  TCl_rleg.TCl=r_Leg.TCl;
  TCl_lfoot.TCl=l_Foot.TCl;
  TCl_rfoot.TCl=r_Foot.TCl;
  TC_overall.TCl=TCo;

// Results for Skin temperatures

  TSkinAll[1] = head.skin.T;
  TSkinAll[2] = chest.skin.T;
  TSkinAll[3] = back.skin.T;
  TSkinAll[4] = pelvis.skin.T;
  TSkinAll[5] = l_Shoulder.skin.T;
  TSkinAll[6] = r_Shoulder.skin.T;
  TSkinAll[7] = l_Arm.skin.T;
  TSkinAll[8] = r_Arm.skin.T;
  TSkinAll[9] = l_Hand.skin.T;
  TSkinAll[10] = r_Hand.skin.T;
  TSkinAll[11] = l_Thigh.skin.T;
  TSkinAll[12] = r_Thigh.skin.T;
  TSkinAll[13] = l_Leg.skin.T;
  TSkinAll[14] = r_Leg.skin.T;
  TSkinAll[15] = l_Foot.skin.T;
  TSkinAll[16] = r_Foot.skin.T;

  // Results for Core temperatures

  TCoreAll[1] = head.core.T;
  TCoreAll[2] = chest.core.T;
  TCoreAll[3] = back.core.T;
  TCoreAll[4] = pelvis.core.T;
  TCoreAll[5] = l_Shoulder.core.T;
  TCoreAll[6] = r_Shoulder.core.T;
  TCoreAll[7] = l_Arm.core.T;
  TCoreAll[8] = r_Arm.core.T;
  TCoreAll[9] = l_Hand.core.T;
  TCoreAll[10] = r_Hand.core.T;
  TCoreAll[11] = l_Thigh.core.T;
  TCoreAll[12] = r_Thigh.core.T;
  TCoreAll[13] = l_Leg.core.T;
  TCoreAll[14] = r_Leg.core.T;
  TCoreAll[15] = l_Foot.core.T;
  TCoreAll[16] = r_Foot.core.T;

  // Temperature of Outer layer of the model:
  // Attention for standalone calculation: The effect of radiation flow is overrated due to radiation modeling: Delta T = (T_skin - T_rad) --> Should be better T_clo - T_rad
  // --> for the coupled simulation, this approach with TOuterLayerAll[i] is okay!

  if head.skin.Icl > 0 then
    TOuterLayerAll[1] = head.skin.T - ((head.skin.Qcon+head.skin.Qrad)*0.155*head.skin.Icl/(head.skin.S*head.skin.fcl*(1-head.skin.CSF)));
  else
     TOuterLayerAll[1] = head.skin.T;
  end if;

  if chest.skin.Icl > 0 then
    TOuterLayerAll[2] = chest.skin.T - ((chest.skin.Qcon+chest.skin.Qrad)*0.155*chest.skin.Icl/(chest.skin.S*chest.skin.fcl*(1-chest.skin.CSF)));
  else
     TOuterLayerAll[2] = chest.skin.T;
  end if;

  if back.skin.Icl > 0 then
    TOuterLayerAll[3] = back.skin.T - ((back.skin.Qcon+back.skin.Qrad)*0.155*back.skin.Icl/(back.skin.S*back.skin.fcl*(1-back.skin.CSF)));
  else
     TOuterLayerAll[3] = back.skin.T;
  end if;

  if pelvis.skin.Icl > 0 then
    TOuterLayerAll[4] = pelvis.skin.T - ((pelvis.skin.Qcon+pelvis.skin.Qrad)*0.155*pelvis.skin.Icl/(pelvis.skin.S*pelvis.skin.fcl*(1-pelvis.skin.CSF)));
  else
     TOuterLayerAll[4] = pelvis.skin.T;
  end if;

  if l_Shoulder.skin.Icl > 0 then
    TOuterLayerAll[5] = l_Shoulder.skin.T - ((l_Shoulder.skin.Qcon+l_Shoulder.skin.Qrad)*0.155*l_Shoulder.skin.Icl/(l_Shoulder.skin.S*l_Shoulder.skin.fcl*(1-l_Shoulder.skin.CSF)));
  else
     TOuterLayerAll[5] = l_Shoulder.skin.T;
  end if;

  if r_Shoulder.skin.Icl > 0 then
    TOuterLayerAll[6] = r_Shoulder.skin.T - ((r_Shoulder.skin.Qcon+r_Shoulder.skin.Qrad)*0.155*r_Shoulder.skin.Icl/(r_Shoulder.skin.S*r_Shoulder.skin.fcl*(1-r_Shoulder.skin.CSF)));
  else
     TOuterLayerAll[6] = r_Shoulder.skin.T;
  end if;

  if l_Arm.skin.Icl > 0 then
    TOuterLayerAll[7] = l_Arm.skin.T - ((l_Arm.skin.Qcon+l_Arm.skin.Qrad)*0.155*l_Arm.skin.Icl/(l_Arm.skin.S*l_Arm.skin.fcl*(1-l_Arm.skin.CSF)));
  else
     TOuterLayerAll[7] = l_Arm.skin.T;
  end if;

  if r_Arm.skin.Icl > 0 then
    TOuterLayerAll[8] = r_Arm.skin.T - ((r_Arm.skin.Qcon+r_Arm.skin.Qrad)*0.155*r_Arm.skin.Icl/(r_Arm.skin.S*r_Arm.skin.fcl*(1-r_Arm.skin.CSF)));
  else
     TOuterLayerAll[8] = r_Arm.skin.T;
  end if;

  if l_Hand.skin.Icl > 0 then
    TOuterLayerAll[9] = l_Hand.skin.T - ((l_Hand.skin.Qcon+l_Hand.skin.Qrad)*0.155*l_Hand.skin.Icl/(l_Hand.skin.S*l_Hand.skin.fcl*(1-l_Hand.skin.CSF)));
  else
     TOuterLayerAll[9] = l_Hand.skin.T;
  end if;

  if r_Hand.skin.Icl > 0 then
    TOuterLayerAll[10] = r_Hand.skin.T - ((r_Hand.skin.Qcon+r_Hand.skin.Qrad)*0.155*r_Hand.skin.Icl/(r_Hand.skin.S*r_Hand.skin.fcl*(1-r_Hand.skin.CSF)));
  else
     TOuterLayerAll[10] = r_Hand.skin.T;
  end if;

  if l_Thigh.skin.Icl > 0 then
    TOuterLayerAll[11] = l_Thigh.skin.T - ((l_Thigh.skin.Qcon+ l_Thigh.skin.Qrad)*0.155*l_Thigh.skin.Icl/(l_Thigh.skin.S*l_Thigh.skin.fcl*(1-l_Thigh.skin.CSF)));
  else
     TOuterLayerAll[11] = l_Thigh.skin.T;
  end if;

  if r_Thigh.skin.Icl > 0 then
    TOuterLayerAll[12] = r_Thigh.skin.T - ((r_Thigh.skin.Qcon+ r_Thigh.skin.Qrad)*0.155*r_Thigh.skin.Icl/(r_Thigh.skin.S*r_Thigh.skin.fcl*(1-r_Thigh.skin.CSF)));
  else
     TOuterLayerAll[12] = r_Thigh.skin.T;
  end if;

  if l_Leg.skin.Icl > 0 then
    TOuterLayerAll[13] = l_Leg.skin.T - ((l_Leg.skin.Qcon+ l_Leg.skin.Qrad)*0.155*l_Leg.skin.Icl/(l_Leg.skin.S*l_Leg.skin.fcl*(1-l_Leg.skin.CSF)));
  else
     TOuterLayerAll[13] = l_Leg.skin.T;
  end if;

  if r_Leg.skin.Icl > 0 then
    TOuterLayerAll[14] = r_Leg.skin.T - ((r_Leg.skin.Qcon+ r_Leg.skin.Qrad)*0.155*r_Leg.skin.Icl/(r_Leg.skin.S*r_Leg.skin.fcl*(1-r_Leg.skin.CSF)));
  else
     TOuterLayerAll[14] = r_Leg.skin.T;
  end if;

  if l_Foot.skin.Icl > 0 then
    TOuterLayerAll[15] = l_Foot.skin.T - ((l_Foot.skin.Qcon+ l_Foot.skin.Qrad)*0.155*l_Foot.skin.Icl/(l_Foot.skin.S*l_Foot.skin.fcl*(1-l_Foot.skin.CSF)));
  else
     TOuterLayerAll[15] = l_Foot.skin.T;
  end if;

  if r_Foot.skin.Icl > 0 then
    TOuterLayerAll[16] = r_Foot.skin.T - ((r_Foot.skin.Qcon+ r_Foot.skin.Qrad)*0.155*r_Foot.skin.Icl/(r_Foot.skin.S*r_Foot.skin.fcl*(1-r_Foot.skin.CSF)));
  else
     TOuterLayerAll[16] = r_Foot.skin.T;
  end if;


// add Coupling heat fluxes
if parameters.UseCoupling then
    // Convective heat flow = convecitve heat flux for each body part from CFD simulation * corrected surface area
    head.skin.Qcon = Input_qcon_head*head.skin.S*head.skin.fcl*(1-head.skin.CSF);
    chest.skin.Qcon = Input_qcon_chest*chest.skin.S*chest.skin.fcl*(1-chest.skin.CSF);
    back.skin.Qcon = Input_qcon_back*back.skin.S*back.skin.fcl*(1-back.skin.CSF);
    pelvis.skin.Qcon = Input_qcon_pelvis*pelvis.skin.S*pelvis.skin.fcl*(1-pelvis.skin.CSF);
    l_Shoulder.skin.Qcon = Input_qcon_l_Shoulder*l_Shoulder.skin.S*l_Shoulder.skin.fcl*(1-l_Shoulder.skin.CSF);
    r_Shoulder.skin.Qcon = Input_qcon_r_Shoulder*r_Shoulder.skin.S*r_Shoulder.skin.fcl*(1-r_Shoulder.skin.CSF);
    l_Arm.skin.Qcon = Input_qcon_l_Arm*l_Arm.skin.S*l_Arm.skin.fcl*(1-l_Arm.skin.CSF);
    r_Arm.skin.Qcon = Input_qcon_r_Arm*r_Arm.skin.S*r_Arm.skin.fcl*(1-r_Arm.skin.CSF);
    l_Hand.skin.Qcon = Input_qcon_l_Hand*l_Hand.skin.S*l_Hand.skin.fcl*(1-l_Hand.skin.CSF);
    r_Hand.skin.Qcon = Input_qcon_r_Hand*r_Hand.skin.S*r_Hand.skin.fcl*(1-r_Hand.skin.CSF);
    l_Thigh.skin.Qcon = Input_qcon_l_Thigh*l_Thigh.skin.S*l_Thigh.skin.fcl*(1-l_Thigh.skin.CSF);
    r_Thigh.skin.Qcon = Input_qcon_r_Thigh*r_Thigh.skin.S*r_Thigh.skin.fcl*(1-r_Thigh.skin.CSF);
    l_Leg.skin.Qcon = Input_qcon_l_Leg*l_Leg.skin.S*l_Leg.skin.fcl*(1-l_Leg.skin.CSF);
    r_Leg.skin.Qcon = Input_qcon_r_Leg*r_Leg.skin.S*r_Leg.skin.fcl*(1-r_Leg.skin.CSF);
    l_Foot.skin.Qcon = Input_qcon_l_Foot*l_Foot.skin.S*l_Foot.skin.fcl*(1-l_Foot.skin.CSF);
    r_Foot.skin.Qcon = Input_qcon_r_Foot*r_Foot.skin.S*r_Foot.skin.fcl*(1-r_Foot.skin.CSF);

    // Radiative heat flow for each body part from CFD simulation
    head.skin.Qrad = Input_qrad_head*head.skin.S*head.skin.fcl*(1-head.skin.CSF);
    chest.skin.Qrad = Input_qrad_chest*chest.skin.S*chest.skin.fcl*(1-chest.skin.CSF);
    back.skin.Qrad = Input_qrad_back*back.skin.S*back.skin.fcl*(1-back.skin.CSF);
    pelvis.skin.Qrad = Input_qrad_pelvis*pelvis.skin.S*pelvis.skin.fcl*(1-pelvis.skin.CSF);
    l_Shoulder.skin.Qrad = Input_qrad_l_Shoulder*l_Shoulder.skin.S*l_Shoulder.skin.fcl*(1-l_Shoulder.skin.CSF);
    r_Shoulder.skin.Qrad = Input_qrad_r_Shoulder*r_Shoulder.skin.S*r_Shoulder.skin.fcl*(1-r_Shoulder.skin.CSF);
    l_Arm.skin.Qrad = Input_qrad_l_Arm*l_Arm.skin.S*l_Arm.skin.fcl*(1-l_Arm.skin.CSF);
    r_Arm.skin.Qrad = Input_qrad_r_Arm*r_Arm.skin.S*r_Arm.skin.fcl*(1-r_Arm.skin.CSF);
    l_Hand.skin.Qrad = Input_qrad_l_Hand*l_Hand.skin.S*l_Hand.skin.fcl*(1-l_Hand.skin.CSF);
    r_Hand.skin.Qrad = Input_qrad_r_Hand*r_Hand.skin.S*r_Hand.skin.fcl*(1-r_Hand.skin.CSF);
    l_Thigh.skin.Qrad = Input_qrad_l_Thigh*l_Thigh.skin.S*l_Thigh.skin.fcl*(1-l_Thigh.skin.CSF);
    r_Thigh.skin.Qrad = Input_qrad_r_Thigh*r_Thigh.skin.S*r_Thigh.skin.fcl*(1-r_Thigh.skin.CSF);
    l_Leg.skin.Qrad = Input_qrad_l_Leg*l_Leg.skin.S*l_Leg.skin.fcl*(1-l_Leg.skin.CSF);
    r_Leg.skin.Qrad = Input_qrad_r_Leg*r_Leg.skin.S*r_Leg.skin.fcl*(1-r_Leg.skin.CSF);
    l_Foot.skin.Qrad = Input_qrad_l_Foot*l_Foot.skin.S*l_Foot.skin.fcl*(1-l_Foot.skin.CSF);
    r_Foot.skin.Qrad = Input_qrad_r_Foot*r_Foot.skin.S*r_Foot.skin.fcl*(1-r_Foot.skin.CSF);
end if;



// Output

  // Temperature of outer layer
  Output_T_Outer_head = TOuterLayerAll[1];
  Output_T_Outer_chest = TOuterLayerAll[2];
  Output_T_Outer_back = TOuterLayerAll[3];
  Output_T_Outer_pelvis = TOuterLayerAll[4];
  Output_T_Outer_l_Shoulder = TOuterLayerAll[5];
  Output_T_Outer_r_Shoulder = TOuterLayerAll[6];
  Output_T_Outer_l_Arm = TOuterLayerAll[7];
  Output_T_Outer_r_Arm = TOuterLayerAll[8];
  Output_T_Outer_l_Hand = TOuterLayerAll[9];
  Output_T_Outer_r_Hand = TOuterLayerAll[10];
  Output_T_Outer_l_Thigh = TOuterLayerAll[11];
  Output_T_Outer_r_Thigh = TOuterLayerAll[12];
  Output_T_Outer_l_Leg = TOuterLayerAll[13];
  Output_T_Outer_r_Leg = TOuterLayerAll[14];
  Output_T_Outer_l_Foot = TOuterLayerAll[15];
  Output_T_Outer_r_Foot = TOuterLayerAll[16];

 // Temperature of skin layer
 Output_T_Skin_head = TSkinAll[1];
 Output_T_Skin_chest = TSkinAll[2];
 Output_T_Skin_back = TSkinAll[3];
 Output_T_Skin_pelvis = TSkinAll[4];
 Output_T_Skin_l_Shoulder = TSkinAll[5];
 Output_T_Skin_r_Shoulder = TSkinAll[6];
 Output_T_Skin_l_Arm = TSkinAll[7];
 Output_T_Skin_r_Arm = TSkinAll[8];
 Output_T_Skin_l_Hand = TSkinAll[9];
 Output_T_Skin_r_Hand = TSkinAll[10];
 Output_T_Skin_l_Thigh = TSkinAll[11];
 Output_T_Skin_r_Thigh = TSkinAll[12];
 Output_T_Skin_l_Leg = TSkinAll[13];
 Output_T_Skin_r_Leg = TSkinAll[14];
 Output_T_Skin_l_Foot = TSkinAll[15];
 Output_T_Skin_r_Foot = TSkinAll[16];

  // Temperature of core layer
 Output_T_Core_head = TCoreAll[1];
 Output_T_Core_chest = TCoreAll[2];
 Output_T_Core_back = TCoreAll[3];
 Output_T_Core_pelvis = TCoreAll[4];
 Output_T_Core_l_Shoulder = TCoreAll[5];
 Output_T_Core_r_Shoulder = TCoreAll[6];
 Output_T_Core_l_Arm = TCoreAll[7];
 Output_T_Core_r_Arm = TCoreAll[8];
 Output_T_Core_l_Hand = TCoreAll[9];
 Output_T_Core_r_Hand = TCoreAll[10];
 Output_T_Core_l_Thigh = TCoreAll[11];
 Output_T_Core_r_Thigh = TCoreAll[12];
 Output_T_Core_l_Leg = TCoreAll[13];
 Output_T_Core_r_Leg = TCoreAll[14];
 Output_T_Core_l_Foot = TCoreAll[15];
 Output_T_Core_r_Foot = TCoreAll[16];


 // Thermal Sensation
 Output_TS_overall = TS_overall.TSl;
 Output_TS_head = TSl_head.TSl;
 Output_TS_chest = TSl_chest.TSl;
 Output_TS_back = TSl_back.TSl;
 Output_TS_pelvis = TSl_pelvis.TSl;
 Output_TS_l_Shoulder = TSl_lshoulder.TSl;
 Output_TS_r_Shoulder = TSl_rshoulder.TSl;
 Output_TS_l_Arm = TSl_larm.TSl;
 Output_TS_r_Arm = TSl_rarm.TSl;
 Output_TS_l_Hand = TSl_lhand.TSl;
 Output_TS_r_Hand = TSl_rhand.TSl;
 Output_TS_l_Thigh = TSl_lthigh.TSl;
 Output_TS_r_Thigh = TSl_rthigh.TSl;
 Output_TS_l_Leg = TSl_lleg.TSl;
 Output_TS_r_Leg = TSl_rleg.TSl;
 Output_TS_l_Foot = TSl_lfoot.TSl;
 Output_TS_r_Foot = TSl_rfoot.TSl;

 // Thermal Comfort
 Output_TC_overall = TC_overall.TCl;
 Output_TC_head = TCl_head.TCl;
 Output_TC_chest = TCl_chest.TCl;
 Output_TC_back = TCl_back.TCl;
 Output_TC_pelvis = TCl_pelvis.TCl;
 Output_TC_l_Shoulder = TCl_lshoulder.TCl;
 Output_TC_r_Shoulder = TCl_rshoulder.TCl;
 Output_TC_l_Arm = TCl_larm.TCl;
 Output_TC_r_Arm = TCl_rarm.TCl;
 Output_TC_l_Hand = TCl_lhand.TCl;
 Output_TC_r_Hand = TCl_rhand.TCl;
 Output_TC_l_Thigh = TCl_lthigh.TCl;
 Output_TC_r_Thigh = TCl_rthigh.TCl;
 Output_TC_l_Leg = TCl_lleg.TCl;
 Output_TC_r_Leg = TCl_rleg.TCl;
 Output_TC_l_Foot = TCl_lfoot.TCl;
 Output_TC_r_Foot = TCl_rfoot.TCl;



  connect(l_Thigh.bloodTherm, centralBlood.bloodTherm3) annotation (Line(
      points={{-6,-49.92},{2,-49.92},{2,-23.5},{-9,-23.5}},
      color={255,186,151},
      pattern=LinePattern.Dot));
  connect(r_Thigh.bloodTherm, centralBlood.bloodTherm2) annotation (Line(
      points={{-17,-51.04},{-22,-51.04},{-22,-26},{-13,-26},{-13,-23.5}},
      color={255,186,151}));
  connect(r_Leg.bloodTherm, centralBlood.bloodTherm2) annotation (Line(
      points={{-18,-63.76},{-24,-63.76},{-24,-26},{-13,-26},{-13,-23.5}},
      color={255,186,151},
      pattern=LinePattern.Dot));
  connect(l_Leg.bloodTherm, centralBlood.bloodTherm3) annotation (Line(
      points={{-3,-62.48},{2,-62.48},{2,-23.5},{-9,-23.5}},
      color={255,186,151},
      pattern=LinePattern.Dot));
  connect(l_Arm.bloodTherm, centralBlood.bloodTherm7) annotation (Line(
      points={{10,-21.92},{6,-21.92},{6,-22},{-6,-22},{-6,-21},{-6.5,-21}},
      color={255,186,151},
      pattern=LinePattern.Dot));
  connect(r_Hand.bloodTherm, centralBlood.bloodTherm2) annotation (Line(
      points={{-33,-31.2},{-30,-31.2},{-30,-24},{-13,-24},{-13,-23.5}},
      color={255,186,151},
      pattern=LinePattern.Dot));
  connect(r_Arm.bloodTherm, centralBlood.bloodTherm5) annotation (Line(
      points={{-32,-19.92},{-20,-19.92},{-20,-21},{-15.5,-21}},
      color={255,186,151},
      pattern=LinePattern.Dot));
  connect(l_Hand.bloodTherm, centralBlood.bloodTherm3) annotation (Line(
      points={{11,-33.92},{6,-33.92},{6,-23.5},{-9,-23.5}},
      color={255,186,151},
      pattern=LinePattern.Dot));
  annotation (Diagram(
      coordinateSystem(preserveAspectRatio=false, extent={{-280,-100},{500,120}}),
                      coordinateparameters(preserveAspectRatio=false, extent={{-100,
            -100},{100,100}}),
      graphics={
        Rectangle(
          extent={{-52,30},{32,-92}},
          lineColor={255,186,151},
          pattern=LinePattern.None,
          fillColor={213,213,213},
          fillPattern=FillPattern.Solid),
        Rectangle(
          extent={{-88,104},{54,34}},
          lineColor={255,186,151},
          pattern=LinePattern.None,
          fillColor={213,213,213},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-28,-80},{14,-88}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={213,213,213},
          fillPattern=FillPattern.None,
          textString="Comfort model"),
        Text(
          extent={{-42,102},{18,96}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={213,213,213},
          fillPattern=FillPattern.None,
          textString="Boundary conditions"),
        Rectangle(
          extent={{146,104},{216,-92}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-37.5,-19.5},{37.5,19.5}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid,
          textString="Local and overall thermal comfort",
          origin={150.5,11.5},
          rotation=90),
        Rectangle(
          extent={{66,104},{138,-92}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-37.5,-19.5},{37.5,19.5}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid,
          origin={70.5,9.5},
          rotation=90,
          textString="Local and overall thermal sensation"),
        Text(
          extent={{-46,62},{30,46}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={213,213,213},
          fillPattern=FillPattern.None,
          textString="order: head, chest, back, pelvis, rshoulder, lshoulder, rarm,
 larm, rhand, lhand, rthigh, lthigh, rleg, lleg, rfoot, lfoot",
          textStyle={TextStyle.Italic}),
        Rectangle(
          extent={{308,104},{392,-92}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-37.5,-19.5},{37.5,19.5}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid,
          origin={314.5,9.5},
          rotation=90,
          textString="Local skin temperature"),
        Rectangle(
          extent={{226,104},{298,-92}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-37.5,-19.5},{37.5,19.5}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid,
          origin={232.5,9.5},
          rotation=90,
          textString="Local outer temperature"),
        Rectangle(
          extent={{402,104},{488,-92}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-37.5,-19.5},{37.5,19.5}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid,
          origin={408.5,9.5},
          rotation=90,
          textString="Local core temperature"),
        Rectangle(
          extent={{-266,104},{-194,-92}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={150,150,150},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-37.5,-19.5},{37.5,19.5}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid,
          origin={-255.5,9.5},
          rotation=90,
          textString="Input data of local convective heat fluxes for each body part
"),     Rectangle(
          extent={{-182,104},{-110,-92}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={150,150,150},
          fillPattern=FillPattern.Solid),
        Text(
          extent={{-37.5,-19.5},{37.5,19.5}},
          lineColor={0,0,0},
          pattern=LinePattern.None,
          fillColor={28,108,200},
          fillPattern=FillPattern.Solid,
          origin={-171.5,9.5},
          rotation=90,
          textString="Input data of local radiative heat fluxes for each body part
")}),                  Icon(coordinateSystem(preserveAspectRatio=false, extent={{-280,
            -100},{500,120}}),
                            graphics={Bitmap(extent={{-60,-64},{62,72}},
          imageSource=""
               + ""
               + "",
          fileName="modelica://ComfortModel33NCM/Pictures/Mensch in Einzelteilen.png")}),
    experiment(StopTime=15600, Interval=60),
    __Dymola_experimentSetupOutput(events=false),
    Documentation(info="<html>
<h4><span style=\"color: #008000\">Overview </span></h4>
<p>33 Node-Comfort-Model NOODEL as standalone model </p>
<h4><span style=\"color: #008000\">Concept </span></h4>
<p>All 16 body parts are connected via the blodd flow.</p>
<p>According to the boundary conditions the body temperatures are calculated. From the body temperatures the thermal sensation and comfort is calculated.</p>
<p>The order of the body parts for the boundary conditions is as follows:</p>
<p>head, chest, back, pelvis, rshoulder, lshoulder, rarm, larm, rhand, lhand, rthigh, lthigh, rleg, lleg, rfoot, lfoot</p>
<p>In the txt file for the input date the following information have to be given:</p>
<p>Tair, Trad, velocity, rh, met, Icl, CSF, Tmt, CSR, ISolar, initial skin temperature, initial core temperature, position</p>
</html>"));
end NOODEL_standalone;
